...
# MAGIC 4. `'hosp' : has_mi ~ has_phenotype + estimated_age + location_source_value + num_visits_after + months_in_EMR_after`
...
for file_name in file_names:
  analyses_pd[file_name] = pd.read_pickle(f"male_infertility_validation/tables/logit_results/after/{file_name}.pkl")
...
for analysis in analyses_pd:
   temp = analyses_pd[analysis].toPandas()
...
primary_vs_crude['log10_OR_has_phenotype_crude'].min()
...
if save:
  ...
  plt.savefig("male_infertility_validation/figures/logit/loglog/fx_vs_crude_aft.pdf", format='pdf', dpi=300, bbox_inches='tight')
  print('Saved.')
...
ax = sns.scatterplot(data=primary_vs_sdoh, x='log10_OR_has_phenotype_primary', y='log10_OR_has_phenotype_sdoh', s=20, hue='sig_overlap', hue_order=['not_significant', 'mi_sig_primary', 'mi_sig_sdoh', 'mi_sig_both', 'con_sig_primary', 'con_sig_both'], palette=['whitesmoke', 'hotpink', 'pink', 'purple', 'dodgerblue', 'midnightblue'], alpha=0.4)
...
if save:
  print('Saving log-log plot...')
  plt.savefig("male_infertility_validation/figures/logit/loglog/fx_vs_sdoh_aft.pdf", format='pdf', dpi=300, bbox_inches='tight')
  print('Saved.')
...
ax = sns.scatterplot(data=primary_vs_hosp, x='log10_OR_has_phenotype_primary', y='log10_OR_has_phenotype_hosp', s=20, hue='sig_overlap', hue_order=['not_significant', 'mi_sig_primary', 'mi_sig_both', 'con_sig_hosp', 'con_sig_both'], palette=['whitesmoke', 'hotpink', 'purple', 'powderblue', 'midnightblue'], alpha=0.4)
...
primary_vs_hosp_no_ns['sig_overlap'] = primary_vs_hosp_no_ns['sig_overlap'].replace({'mi_sig_primary' : 'mi - primary only',
                                                                                     'mi_sig_both' : 'mi - both',
                                                                                     'con_sig_hosp' : 'con - hosp. util. only',
                                                                                     'con_sig_both' : 'con - both'})
...
plt.legend(loc=(0.02,0.775), fontsize=14)

if save:
  print('Saving log-log plot...')
  plt.savefig("male_infertility_validation/figures/logit/loglog/fx_vs_hosp_aft.pdf", format='pdf', dpi=300, bbox_inches='tight')
  print('Saved.')
...
if save:
    ...
    plt.savefig("male_infertility_validation/figures/logit/upset/fx_upset_mi_aft.pdf", format='pdf', dpi=300, bbox_inches='tight')
    print('Saved.')
plt.show()
...
if save:
  print('Saving Upset plot...')
  plt.savefig("male_infertility_validation/figures/logit/upset/fx_upset_con_aft.pdf", format='pdf', dpi=300, bbox_inches='tight')
  print('Saved.')
plt.show()
...
for sig_overlap in all_sig['Significance_MI'].unique():
  if sig_overlap != ():
    ...
    if save:
      ...
      print(f"Saving mi_after_{overlap_folder_name}")
      temp.to_csv(f"male_infertility_validation/tables/upset/mi_after_{overlap_folder_name}.csv")
      print('Saved.\n')
  ...
  if sig_overlap != ():
    ...
    if save:
      ...
      for overlap in sig_overlap:
        overlap_names.append(overlap[16:])
      overlap_folder_name = '_'.join(overlap_names)
      print(f"Saving con_after_{overlap_folder_name}")
      temp.to_csv(f"male_infertility_validation/tables/upset/con_after_{overlap_folder_name}.csv")
      print('Saved.\n')
