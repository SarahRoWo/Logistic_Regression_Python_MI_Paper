...
# MAGIC 1. `'crude' : has_pi ~ has_phenotype` 
# MAGIC 2. `'primary' : has_pi ~ has_phenotype + estimated_age + location_source_value` 
# MAGIC 3. `'sdoh' : has_pi ~ has_phenotype + estimated_age + location_source_value + race + ethnicity + ADI` 
# MAGIC 4. `'hosp' : has_pi ~ has_phenotype + estimated_age + location_source_value + num_visits_after + months_in_EMR_after` 
# MAGIC 5. `'hosp_ln' : has_pi ~ has_phenotype + estimated_age + location_source_value + ln_num_visits_after + ln_months_in_EMR_after` 
...
# MAGIC %md
# MAGIC ### Annotate top ORs for volcano plots (both control and male infertility)

# COMMAND ----------

analyses_pd['primary'].info()
...
for analysis in analyses_pd:
  ...
  mi_annotate = temp2.sort_values(by='log10_OR_has_phenotype', ascending=False).head(7)
  con_annotate = temp2.sort_values(by='log10_OR_has_phenotype').head(3)
...
volcano(analyses_pd['primary'], 
        ...
        save=True,
        file_name_save='male_infertility_validation/figures/logit/volcano/fx_vol_aft')
...
# MAGIC %md
# MAGIC ### Annotate the 10 phenotypes with lowest p-values for patients with paternal infertility
...
# MAGIC %md
# MAGIC ### Manhattan plot for primary analysis - focusing on phenotypes associated with male infertility
...
fig = mhat_no_line_breaks(df=analyses_visualize_manhattan['primary'], # pandas DataFrame
                          groupby_col='phecode_category', # column to group by
                          logp='-log10_pval_has_phenotype', # df column containing -log 10 of the p-value
                          
                          # Figure parameters
                          dim=(20,10), 
                          
                          sig_pval_line=True, # bool that adds significance cutoff line
                          sig_pval=cutoff_pval, # significant p-value cutoff
                          
                          axxlabel=None, # x-axis label for bottom subplot 
                          axylabel=None, # y-axis label (used?)
                          axlabelfontsize=31, # x-axis label fontsize for bottom subplot
                          axlabelfontname="DejaVu Sans", # font used
                          axtickfontsize=28, # x-axis tick label fontsize
                          axtickfontname="DejaVu Sans", # font for xticks 
                          ar=90, # How much to rotate phecode category name on x axis
                          
                          dotsize=25, # size of markers
                          figtitle=f"Male Infertility vs. Vasectomy\nManhattan Plot\n", # figure title
                          
                          # Annotation parameters
                          annotate=True, # Whether to annotate
                          annotatefontsize=25, # annotation font size
                          ...
                          # Save parameters
                          save=False,
                          filename="fx_mhat_aft")
print('Saving outside function...')
fig.savefig("male_infertility_validation/figures/logit/manhattan/fx_mhat_aft.pdf", format='pdf', dpi=300, bbox_inches='tight')
print('Saved.')
