...
# MAGIC %md
# MAGIC ## 20230510 Compare diagnoses for patients with paternal infertility vs patients who do not have paternal infertility diagnosis
...
# MAGIC %md
# MAGIC ## Filter for phenotypes after male infertility diagnosis or vasectomy procedure
...
diag_combined = combine_diagnoses(df_case=mi_phe_after, df_con=vas_only_phe_after)
demo_combined = combine_demographics(mi_demo=mi_demo, con_demo=vas_only_demo)

# COMMAND ----------

# MAGIC %md
# MAGIC #### Only keep phenotypes that overlap for paternal infertility and control patients

# COMMAND ----------

phenotypes_both = set(mi_phe_after['phenotype'].unique()) & set(vas_only_phe_after['phenotype'].unique())
...
mi_phe_b4 = mi_phe[mi_phe['phe_time_after'] == 1]
vas_only_phe_b4 = vas_only_phe[vas_only_phe['phe_time_after'] == 1]
...
#phecodes = pd.read_pickle("male_infertility_validation/phecodes/phecodes_cat_v2.pkl")
...
for analysis in results_final:
  ...
  temp = results_final[analysis]
  temp.to_pickle(f"male_infertility_validation/tables/logit_results/after/{analysis}.pkl")
  print('Done.\n')
